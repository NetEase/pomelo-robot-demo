...
Server.prototype = {
    ...
    announce_node: function(socket, message) {
        ...
        socket.on('report', function(message) {
            //rserver.io.sockets.in('web_clients').emit('statusreport',{status:rserver.status,conf:rserver.runconfig});
    ...
    announce_web_client: function(socket) {
        ...
        setInterval(function(){
				...
				socket.on('webreport', function(message) {
						if (!!rserver.runconfig) {
							socket.emit('webreport',stat.getData(rserver));
						}
        });
